...
![$$ X= HZH $$](img/502577_1_En_4_Chapter_TeX_Equi.png)T2】
...
∣*β*T2⟩态被称为*三重态*。有三种三重态。另外两个是∣00⟩和|11⟩.∣*β*t8】11⟩被称为*单态*。清单 4-1f 显示了一个构建铃态的量子电路，它打印铃测量计数，然后生成一个直方图。
...
方程 4.10 表明，要获得关于一个纠缠系统的最多信息，我们需要考虑*整个*系统；也就是说， *S* ( *ρ* )小于简化密度矩阵的组合熵，因为当考虑整个系统时知识增加了。拥有简化密度矩阵*ρ*T10】1 和*ρ*2<sub>2</sub>的用户在仅考虑系统的部分时对状态的了解较少。
...
![$$ \mid GHZ\Big\rangle =\frac{\mid 000\left\rangle +\mid 111\right\rangle }{\sqrt{2}} $$](img/502577_1_En_4_Chapter_TeX_Equah.png)T2】
...
*   方程 4.25 中的∣*b*T2】00⟩有下标 00，表示按照惯例，所有量子位都在∣0⟩.状态初始化
...
注意，传送状态是*而不是叠加*状态。因此，任何任意态的∣*b*T4】1⟩都可以通过图 4-15 方案进行隐形传送。Alice 测量她的侧信道数据，并通过经典信道将其发送给 Bob。鲍勃使用图 4-15 中经典控制的泡利 X(或 CNOT)和 Z 门重建量子数据。这种情况本质上是安全的，因为
...
图 4-19 显示了酉变换，其中一个假想的酉变换*U*<sub>T5】f</sub>作用于两个量子位，使得

![$$ {U}_f\mid x,y\left\rangle =\mid x,y\oplus f(x)\right\rangle, x,y\in \left\{0,1\right\} $$](img/502577_1_En_4_Chapter_TeX_Equau.png)

(4.37)

对方程 4.37 的解释表明，第一量子位保持不受*U*<sub>T5】f</sub>作用的影响。而第二量子位经受*异或*运算(由⊕算子给出)。
...
![$$ =\frac{1}{\sqrt{2}}\left(|0,0\oplus f(0)\Big\rangle +|1,0\oplus f(1)\Big\rangle \right) $$](img/502577_1_En_4_Chapter_TeX_Equaz.png)T2】
