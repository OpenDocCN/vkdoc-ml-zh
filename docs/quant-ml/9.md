...
让事情回到原点:薛定谔方程，由![$$ i\mathit{\hslash}\frac{d\mid \left.\psi (t)\right\rangle }{dt}=\mathcal{H}\mid \left.\psi (t)\right\rangle $$](img/502577_1_En_9_Chapter_TeX_IEq1.png)给出，可以用来获得拉普拉斯。 <sup>[ 2 ](#Fn2)</sup> 我们之前遇到过拉普拉斯算子，尤其是在第 [8](8.html) 章中解决 NP 难的旅行售货员问题时。薛定谔方程左边的指数算符通常用![$$ \mathcal{H} $$](img/502577_1_En_9_Chapter_TeX_IEq2.png)中对应的幂级数来定义。通过泛函微积分的性质，下面的算子描述了我们广为使用的著名酉算子:![$$ U={e}^{-i\mathcal{H}t} $$](img/502577_1_En_9_Chapter_TeX_IEq3.png)。从数学和物理学的角度来看，这意味着拉普拉斯算子保持了系统状态的规范化。
...
![$$ p\left(x,t\right)={\left|\left\langle x|{e}^{- iLt}|\psi (0)\right\rangle \right|}^2 $$](img/502577_1_En_9_Chapter_TeX_Equj.png)

(9.6)

其中初始位置∣ *ψ* (0)⟩ = ∣ 0⟩)由下式给出

![$$ {\left|\left\langle x|{e}^{- iLt}|0\right\rangle \right|}^2={\left|{J}_x(2t)\right|}^2 $$](img/502577_1_En_9_Chapter_TeX_Equk.png)
...
![$$ {e}^{-i\sum \limits_n{\mathcal{H}}_nt}=\prod \limits_n{e}^{-i{\mathcal{H}}_nt}+O\left({t}^2\right) $$](img/502577_1_En_9_Chapter_TeX_Equm.png)

(9.8)

现在，如果我们将这个方程进行三重化(见第 7 章，方程 [7)。24](7.html#Equar) ，其陈述了对于小的 *t* ，因式分解规则近似有效，我们可以将![$$ \mathcal{H} $$](img/502577_1_En_9_Chapter_TeX_IEq19.png)对于时间 *t* 的演变写成长度为δ*t*的小时间步长的序列，如
...
![$$ {\mathrm{argmin}}_w\left[\frac{1}{N}\sum \limits_{i=1}^N{\left(\sum \limits_{k=1}^K{w}_k{h}_k\left({x}_i\right)-{y}_i\right)}^2+\lambda {\left\Vert w\right\Vert}_0\right] $$](img/502577_1_En_9_Chapter_TeX_Equo.png)

其中*h*<sub>*k*</sub>(*x*<sub>*I*</sub>)为训练实例 *k* 对弱学习者的预测。这个公式中的权重是二进制的。因此，这个目标函数默认映射到伊辛模型。正则化确保了稀疏性。它不是经典的正则化；在数字计算机上很难优化这一项。

扩展目标的二次部分，我们有

![$$ {\mathrm{argmin}}_w\left[\frac{1}{N}\sum \limits_{i=1}^N{\left(\sum \limits_{k=1}^K{w}_k{h}_k\left({x}_i\right)\right)}^2-2\sum \limits_{k=1}^K{w}_k{h}_k\left({x}_i\right){y}_i+{y_i}^2+\lambda {\left\Vert w\right\Vert}_0\right] $$](img/502577_1_En_9_Chapter_TeX_Equp.png)

其中，*y*<sub>*I*</sub><sup>2</sup>为常数。因此，优化简化为

![$$ {\mathrm{argmin}}_w\left[\frac{1}{N}\sum \limits_{k=1}^K\sum \limits_{l=1}^K{w}_k{w}_l\left(\sum \limits_{i=1}^N{h}_k\left({x}_i\right){h}_l\left({x}_i\right)\right)-\frac{2}{N}\sum \limits_{k=1}^K{w}_k\sum \limits_{i=1}^N{h}_k\left({x}_i\right){y}_i+{y_i}^2+\lambda {\left\Vert w\right\Vert}_0\right] $$](img/502577_1_En_9_Chapter_TeX_Equq.png)
