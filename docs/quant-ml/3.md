...
其中，*x*<sub>T3】IT5】为输入， *w* <sub>*i*</sub> 为权重， *b* 为偏差， *z* 为 *logit* 或*加权和*。输入和输出要么是实数，要么是二进制数。等式 3.2 定义了用非线性得出的决策。可以将等式 3.1 中的偏差吸收为权重，使得可以仅使用更新权重的规则。如图 3-1 所示。</sub>
...
为了实现这一点，输入*x*T2 0 被添加到等式 3.1 中，值为 1，偏差作为其权重。
...
通常，一个神经网络由 *N 个*输入组成，比如说*I*<sub>T5】N</sub>和 *N* 权值如图 3-2 所示。
...
图 3-14 显示了一个具有两个隐藏层的前馈神经网络。前馈神经网络是通过连接多层感知器构成的，使得每层的每个单元都连接到下一层的单元。第一层由输入*x*<sub>T5】I</sub>组成，下一层是由 *h* <sub>*i*</sub> 单元组成的隐藏层，最后一层有输出 *y* <sub>*i*</sub> 。有一个激活函数来更新每个神经元，每一层都在前一层之后更新，从而形成一个*前馈*神经网络。以下来自方程 3.6 ，图 3-11 的单隐层前馈神经网络模型。
...
**步骤 13** :激活功能:ReLU(整流器线性单元)。为了在节点中施加激活函数，第一步是定义输入的线性和。【T2![$$ Y=W.X+b $$](img/502577_1_En_3_Chapter_TeX_Equl.png)

第二步是定义激活函数输出。

![$$ Z=f(Y) $$](img/502577_1_En_3_Chapter_TeX_Equm.png)T2】
