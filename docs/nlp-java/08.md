...
为了执行这个分析，我们需要使用一个情感`annotator`，如下面的代码所示。这也需要使用`tokenize`、`ssplit`和`parse`标注器。`parse`注释器提供了更多关于文本的结构信息，这将在[第 10 章](part0201.html#5VM120-447d219d688d46cb9ed55b88cf17edcf)、*使用解析器提取关系*中详细讨论:
